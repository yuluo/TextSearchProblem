...
/**
 * Iterator class which returns token strings from the input string. It must be initialized with a
 * regular expression representing a word. Returned tokens will alternate between words (i.e. that
 * match the regular expression) and intervening strings that are not words.
 *
 * <p>The method isWord(String) can be used to check whether a given token is a word or an
 * intervening token.
 *
 * <p>To learn what regular expression syntax is supported by the Java platform, see the
...
public class TextTokenizer implements Iterator<String> {
  ...
  /**
   * Initializes the tokenizer with an input string and a regular expression representing a word.
   *
   * @param input
   * @param wordRegex
   */
  ...
  /** Returns true if more tokens are available, i.e. if end-of-file has not been reached. */
  ...
  /**
   * Returns true if the string matches this tokenizer's regular expression, otherwise returns
   * false.
   */
